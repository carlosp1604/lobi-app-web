...
export const createTeamParticipantsSpecSchema = (config: TeamParticipantsSpecSchemaDto, t: Translate) => {
  ...
  return TeamParticipantsSchema.superRefine((val, ctx) => {
    ...
    if (minTeams < configMinTeams || minTeams > configMaxTeams) {
      ctx.addIssue({
        code: 'custom',
        message: t('team_participants_min_teams_invalid_range_message_title', { min: configMinTeams, max: configMaxTeams }),
        path: ['minTeams']
      });
    }
    ...
    if (maxTeams < configMinTeams || maxTeams > configMaxTeams) {
      ctx.addIssue({
        code: 'custom',
        message: t('team_participants_max_teams_invalid_range_message_title', { min: configMinTeams, max: configMaxTeams }),
        path: ['maxTeams']
      });
    }
    ...
    if (playersPerTeam < configMinPlayersPerTeam || playersPerTeam > configMaxPlayersPerTeam) {
      ctx.addIssue({
        code: 'custom',
        message: t('team_participants_players_per_team_invalid_range_message_title', { min: configMinPlayersPerTeam, max: configMaxPlayersPerTeam }),
        path: ['playersPerTeam']
      });
    }
