...
export const GeographicCapabilityViewer = ({ capability }: GeographicCapabilityViewerProps) => {
  ...
  if (type === 'geographic_point') {
    const location = parseCoords(data.lat, data.lng);
    if (!location) {
      return null;
    }
  ...
  if (type === 'geographic_range') {
    const startLocation = parseCoords(data.start?.lat, data.start?.lng);
    const endLocation = parseCoords(data.end?.lat, data.end?.lng);
