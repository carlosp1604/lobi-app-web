...
import { joinableStatus, leaveableStatus, manageableStatus, participableStatus} from "~/types/activity/ActivityStatus";
import {
  ActivityDetailsDto,
  ActivityHostDto,
...
export const ActivityHeader = ({
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col md:flex-row md:items-start justify-between gap-6 border-b pb-6">
        <div className="space-y-3.5 flex-1 w-full">
          <div className="flex items-center gap-2">
            <Badge variant="secondary" className="uppercase bg-orange-50 text-orange-700 h-5 px-2">
              {t(`sports_${sport.slug}_title`)}
            </Badge>

            <Badge variant="outline" className={`uppercase h-5 px-2 font-bold ${statusColorMap[activity.status]}`}>
          ...
          <div className="pt-1">
            <Field>
              ...
              <Progress
                ...
                className="h-2 [&>div]:bg-black bg-muted rounded-lg"
              />
            </Field>
          ...
          <div className="flex flex-wrap gap-1 pt-1">
            {activity.levels.map((level) => (
              <Badge
                key={level.id}
                variant="outline"
                className="p-3 uppercase text-muted-foreground"
              >
        ...
        <div className="shrink-0 w-full md:w-auto flex flex-col gap-2 md:mt-1">
          {isHost && manageableStatus.includes(activity.status) && (
            <Button
              variant="destructive"
              className="w-full md:w-32 font-medium cursor-pointer"
              onClick={onCancel}
              disabled={loading}
            >
          ...
          {isParticipant && leaveableStatus.includes(activity.status) && (
            <Button
              variant="outline"
              className="w-full md:w-32 font-medium cursor-pointer"
              onClick={onLeave}
          ...
          {!isHost && !isParticipant && joinableStatus.includes(activity.status) && (
            <Button
              className="w-full md:w-32 font-medium cursor-pointer"
              onClick={onJoin}
              disabled={loading}
      ...
      {isParticipant && participation && !isHost && participableStatus.includes(activity.status) && (
