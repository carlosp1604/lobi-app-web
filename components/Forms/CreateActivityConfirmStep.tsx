          ...
          { Object.keys(values.capabilities || {}).length > 0 ? (
            <div className="flex flex-col gap-y-5">
              { Object.entries(values.capabilities).map(([key, value]) => {
                ...
                let formattedItems: Array<string> = []

                if (!capabilityConfig) {
                  formattedItems = [typeof value === 'object' ? JSON.stringify(value) : String(value)]
                } else if (key === 'location' || key === 'location_range') {
                  formattedItems = formatGeographicCapabilityData(
                    value as GeographicCapabilityDto,
                    capabilityConfig as GeographicCapabilitySchemaDto,
                    t
                  )
                } else if (key === 'ranking') {
                    ...
                    <ul className="flex flex-col gap-1.5 mt-1">
                      { formattedItems.map((item, index) => (
                        <li
                          // eslint-disable-next-line @eslint-react/no-array-index-key
                          key={ `${key}_item_${index}` }
                          className="text-sm font-medium text-foreground flex items-start gap-2 mask-circle">
                          <div className="h-1.5 w-1.5 rounded-full bg-primary/50 shrink-0 mt-1.5"/>
                          <span>{ item }</span>
                        </li>
                      )) }
